    ...
    <div className='container'>
      <table>
        ...
        <tbody>{tableData.body.map((row, ri) =>
          <tr key={ri} >
            {row.map((cell, ci) => <td key={ci} draggable={cell.selected} onDragStart={drag} onDrop={drop} onDragOver={allowDrop}>
              {ci === 0 && <div className='optbtn'>
              ...
              <input value={cell.value} className={cell.selected? 'selected': ''} onChange={(e) => handleInput(e, ci, ri)} disabled={cell.disabled} /></td>)}
          </tr>
        )}</tbody>
      </table>
